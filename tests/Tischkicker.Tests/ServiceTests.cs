...
public class MatchControlServiceTests : IDisposable
{
    ...
    public MatchControlServiceTests()
    {
        var notifier = new LiveNotifier();
        _control = new MatchControl(_dbf, notifier, new TournamentSetup(_dbf, notifier));
    }
...
public class TournamentSetupServiceTests : IDisposable
{
    ...
    public TournamentSetupServiceTests()
    {
        ...
        _control = new MatchControl(_dbf, notifier, _setup);
    }
    ...
    [Fact]
    public void KnockoutPhase_AutoGeneratedAfterLastGroupMatch_WiresFinal()
    {
        var ids = Teams(8);
        var t = _setup.CreateTournament("Cup", TournamentFormat.Groups, 1, 360);
        _setup.SetTeams(t.Id, ids);
        ...
        // Nach dem letzten Gruppenspiel wird die K.o.-Phase automatisch erzeugt.
        using var db = _dbf.CreateDbContext();
        var bracket = db.Matches.Where(m => m.TournamentId == t.Id && m.GroupName == null).ToList();
        ...
        // Ein erneuter manueller Aufruf ist blockiert (bereits erzeugt).
        Assert.Throws<TournamentSetupException>(() => _setup.GenerateKnockoutPhase(t.Id));
    }

    [Fact]
    public void KnockoutPhase_NotGeneratedWhileGroupMatchesOpen()
    {
        var ids = Teams(8);
        var t = _setup.CreateTournament("Cup", TournamentFormat.Groups, 1, 360);
        _setup.SetTeams(t.Id, ids);
        var groupMatches = _setup.Generate(t.Id);

        // Alle bis auf ein Gruppenspiel beenden – noch keine K.o.-Phase.
        foreach (var m in groupMatches.Take(groupMatches.Count - 1))
        {
            _control.AdjustScore(m.Id, "a", 1);
            _control.Finish(m.Id, MatchControl.NowMs());
        }
        using var db = _dbf.CreateDbContext();
